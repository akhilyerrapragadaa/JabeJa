...
public class Jabeja {
  ...
  public Node findPartner(int nodeId, Integer[] nodes){
    ...
    double highestBenefit = 0;
     ...
     for(int eachId : nodeIds){
      Node nodeq = entireGraph.get(eachId);

      int dpp = this.getDegree(nodep, nodep.getColor());
      int dqq = this.getDegree(nodeq, nodeq.getColor());
      double old =  Math.pow(dqq, config.getAlpha())+ Math.pow(dpp, config.getAlpha());
      
      int dpq = this.getDegree(nodep, nodeq.getColor());
      int dqp = this.getDegree(nodeq, nodep.getColor());
      double newVal = Math.pow(dpq, config.getAlpha())+ Math.pow(dqp, config.getAlpha());
